...
module.exports = {
  ...
  find: async function (req, res) {
    ...
    try {
      ...
      FROM listing_assign_proc_v WHERE 1=1
     ...
     (SELECT TOP 1 CONVERT(VARCHAR(16),tanggal_gi,120)
      ...
      FROM listing_assign_proc_v ltbv WHERE 1=1 ${WhereBundleId} ${whereRange} ${WherePlannerId} ${WhereTranspoterId} ${WhereKodeStatus} ${filtersearchtext}
      ...
      if(count == 0){
       ...
       (SELECT TOP 1 CONVERT(VARCHAR(16),tanggal_gi,120)
        ...
        FROM listing_assign_proc_v ltbv WHERE 1=1
        ${WhereBundleId} ${whereRange} ${WherePlannerId} ${WhereTranspoterId} ${WhereKodeStatus} ${filtersearchtext}
        ORDER BY tanggal_penjemputan ASC`;
  ...
  findJumlahAssignProc: async function(req, res) {
    await DB.poolConnect;
    try {
      const request = DB.pool.request();

      let nip = req.param("id");
      let queryDataTable = `SELECT COUNT(1) AS jumlah FROM listing_assign_proc_v`;
  ...
  assignProcurement: async function (req, res) {
    ...
    try {
      ...
      if(checkMenggunkanConsoleAtauTidak){
        ...
        let updadateDataDo = `UPDATE delivery_order 
        SET catatan_assign_proc = 'Data di assign ke procurement oleh ${olehUser}',
        tanggal_assign_proc = getdate(),
        isproc='Y'
...
updateDataTransporter: async function (req, res) {
  ...
  try {
    ...
    let sqlGetDataDoSpesifik = `SELECT r_kendaraan_id FROM listing_assign_proc_v WHERE nomor_id = '${nomor_id}'`;
    ...
    if(totalData == 0){
      ...
      request.query(queryUpdate, async (err, result) => {
        ...
        let sqlgetdo = `SELECT * FROM listing_assign_proc_v WHERE nomor_id = '${nomor_id}'`;

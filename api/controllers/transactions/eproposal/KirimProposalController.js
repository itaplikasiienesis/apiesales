...
module.exports = {
  ...
  kirim: async function(req, res) {
    ...
    try {
      ...
      LEFT JOIN m_division mdv ON mdv.division_code = p.division_code
      ...
      for (let i = 0; i < budget.length; i++) {
        ...
        let sqlgetbudgetperactivity = `SELECT SUM(budget) as total FROM proposal_budget 
        ...
        ${companyfilter}
        AND activity.group_id = (SELECT group_id FROM m_activity activity 
        WHERE activity_code = '${activity_code}' 
        AND activity.year = '${budget_year}'
        AND active=${active} ${companyfilter})`;
    ...
    //   let sku = _.uniqBy(datavariant,"variant_id");
      ...
      let file = datafile[0];
        ...
        let total_budget = row.total_budget;
        let region_id = row.region_id;
        let activity_code = row.budget.map(function (item) {
        ...
        let branchcode = row.budget.map(function (item) {
          return item['branch_code'];
        });
        // console.log(workflow_id);
        let brandcode = row.budget.map(function (item) {
            return item['brand_code'];
        });

        let distributorid = row.distributor.map(function (item) {
        ...
        if(approval.length==1){
          ...
          let workflow_id = approval.length > 0 ? `'${approval[0].workflow_id}'` : `NULL`;
          let sqlUpdateWorkFlowid = `UPDATE proposal SET workflow_id = ${workflow_id},status_id=10 WHERE proposal_id=${proposal_id}`;
          await request.query(sqlUpdateWorkFlowid);
          ...
          if(employee.length){
            ...
            VALUES(${proposal_approval_id}, ${proposal_id},'${employee_id}',1,${no_appr},${flag},'${token}','${row.created_by}',now(),'${row.created_by}',now())`;
            //.log(SqlInsertDataEmployeeProposal);

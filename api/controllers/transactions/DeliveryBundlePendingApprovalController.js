...
module.exports = {
    ...
    find: async function (req, res) {
        ...
        try {
          ...
          FROM listing_approval_delivery_order_bundle_v WHERE 1=1 ${WhereNip}
         ...
         (SELECT TOP 1 CONVERT(VARCHAR(16),tanggal_gi,120)
          ...
          FROM listing_approval_delivery_order_bundle_v ltbv WHERE 1=1 ${WhereNip} ${WhereBundleId} ${whereRange} ${WherePlannerId} ${WhereTranspoterId} ${WhereKodeStatus} ${filtersearchtext}
          ...
          if(count == 0){
           ...
           (SELECT TOP 1 CONVERT(VARCHAR(16),tanggal_gi,120)
            ...
            FROM listing_approval_delivery_order_bundle_v ltbv WHERE 1=1 ${WhereNip}
            ${WhereBundleId} ${whereRange} ${WherePlannerId} ${WhereTranspoterId} ${WhereKodeStatus} ${filtersearchtext}
            ORDER BY tanggal_penjemputan ASC`;
      ...
      findPlacedOrderAll: async function (req, res) {
        ...
        try {
          ...
          let queryCountTable = `SELECT COUNT(1) AS total_rows
          FROM listing_approval_delivery_order_bundle_v WHERE 1=1 ${WhereBundleId} 
          ...
          let queryDataTable = `SELECT * FROM listing_approval_delivery_order_bundle_v WHERE 1=1 ${WhereBundleId} ${whereRange} ${WherePlannerId} ${WhereTranspoterId} ${WhereKodeStatus} ${filtersearchtext}
          ORDER BY tanggal_penjemputan ASC
          OFFSET ${offset} ROWS
          FETCH NEXT ${limit} ROWS ONLY`;
          ...
          if(count == 0){
            ...
            queryDataTable = `SELECT * FROM listing_approval_delivery_order_bundle_v WHERE 1=1
            ${WhereBundleId} ${whereRange} ${WherePlannerId} ${WhereTranspoterId} ${WhereKodeStatus} ${filtersearchtext}
            ORDER BY tanggal_penjemputan ASC`;
      ...
      findOne: async function (req, res) {
        ...
        try {
          ...
          let dataroute = await pool.request().query(sqlGetRoute);
          ...
          for (let o = 0; o < dataRoute.length; o++) {
              const lokasi_pickup = dataRoute[o].lokasi_pickup;
              const kota_destinasi = dataRoute[o].kota_destinasi;

              arrayLokasiPickup.push(lokasi_pickup);
              arrayKotaDestinasi.push(kota_destinasi);
          ...
          let lokasi_pickup = arrayLokasiPickup.length > 0 ? arrayLokasiPickup.toString() : '';
          let kota_destinasi = arrayKotaDestinasi.length > 0 ? arrayKotaDestinasi.toString(): '';


          // console.log('lokasi_pickup ',lokasi_pickup);
          // console.log('kota_destinasi ',kota_destinasi);
          ...
          request.query(queryDataTable, async (err, result) => {
            ...
            let sqlGetNamaDistributor = `SELECT * FROM delivery_order WHERE bundle_id='${bundle_id}' AND isactive='Y'`;
            ...
            // console.log(data);
            // console.log(arrayTujuan);
            // console.log(arrayAlamat);
            // console.log(arrayRegion);
            // console.log(arrayChannel);
            // console.log(arrayPenerima);
            // console.log(arrayTanggalSampaiTujuan);
            // console.log(arrayDriver);
            // console.log(arrayNomorHpsistenDriver);
            // console.log(arrayNomorDo);
            // console.log(arrayNomorSo);
            // console.log(arrayPlatNomorKendaraan);
      ...
      konfirmLogistik: async function (req, res) {
        ...
        try {
            ...
            let sqlGetDo = `SELECT delivery_order_id FROM delivery_order WHERE bundle_id = '${bundle_id}'`;
            ...
            if(jumlah_data > 0){
              ...
              let sqlGetDo = `SELECT delivery_order_id FROM delivery_order WHERE console_number = '${bundle_id}'`;
              let datado = await request.query(sqlGetDo) // --> execute
              let datadeliveryorder = datado.recordset // --> ambil recordset nya aja
              delivery_order_id = datadeliveryorder.length > 0 ? datadeliveryorder[0].delivery_order_id : null;
                      ...
                      if (getConsolNumber == null || !getConsolNumber ) {
                        ...
                        if(delivery_bundle_approval_id){
                          ...
                          if(delivery_bundle_approval_next_id){
                            ...
                            const updateDataDo = `UPDATE delivery_order SET status = 'Waiting Approval' WHERE bundle_id = '${getBundle}'  `
                            await request.query(updateDataDo);
                          
                          }else{

                            // JIKA PROSES HIRARKI APPROVAL SUDAH HABIS
                            const updateDataDo = `UPDATE delivery_order set kode_status = 'WT1' , status = 'Placed Order' WHERE bundle_id = '${getBundle}'  `
                            await request.query(updateDataDo);
                        ...
                        if(delivery_bundle_approval_id){
                          ...
                          if(delivery_bundle_approval_next_id){
                            ...
                            // JIKA PROSES HIRARKI APPROVAL SUDAH HABIS
                            const updateDataDo = `UPDATE delivery_order SET kode_status = 'WT1' , status = 'Placed Order' WHERE console_number = '${getConsolNumber}'  `
                            await request.query(updateDataDo);
      ...
      rejectLogistik: async function (req, res) {
        ...
        try {
            const request = DB.pool.request(); //--> init var request koneksi            
            let sqlGetDo = `SELECT delivery_order_id FROM delivery_order WHERE bundle_id = '${bundle_id}' OR console_number = '${bundle_id}'`;
            let datado = await request.query(sqlGetDo) // --> execute
            let datadeliveryorder = datado.recordset // --> ambil recordset nya aja
            let nip = m_user_id;
            ...
            if(delivery_order_id){
                    ...
                    if (jml > 0) {
                      ...
                      if (getConsolNumber == null || !getConsolNumber ) {
                        ...
                        const updateDataDo = `UPDATE delivery_order SET kode_status = 'RJF' , status = 'Reject', updatedby = '${m_user_id}', updated = GETDATE(),isactive='N' WHERE bundle_id = '${getBundle}'`;
                        await request.query(updateDataDo);
                        ...
                        WHERE console_number = '${getConsolNumber}'`;
                        await request.query(updateDataDo);
...
updateDataTransporter: async function (req, res) {
  const { m_user_id, bundle_id, console_number,m_transporter_id } = req.body;
  ...
  try {
    ...
    let sqlGetDataDoSpesifik = `SELECT r_kendaraan_id FROM delivery_order WHERE bundle_id = '${bundle_id}'`;
    ...
    let queryUpdate = `UPDATE delivery_order SET m_transporter_id = ${m_transporter_id_text},kode_status = 'DOD',status='Draft',updatedby = ${m_user_id_text},
  ...
  updateBundleAndConsole: async function (req, res) {
    ...
    try {
      ...
      if(newEditBundleId != bundle_id || newEditConsoleNumber != console_number){
        ...
        WHERE console_number = '${console_number}' AND bundle_id = '${bundle_id}' `;

        //console.log(queryUpdate);

        request.query(queryUpdate, async (err) => {
          if (err) {
            return res.error(err);
          }
  ...
  findJumlahApprovalBidding: async function(req, res) {
    ...
    try {
      ...
      // let queryDataTable = `SELECT COUNT(1) AS jumlah FROM delivery_bundle_approval dba WHERE nip = '${nip}' 
      // AND flag = 0 AND isstatus = 1 AND dba.isactive = 'Y'
      // AND (
      //   SELECT count(1) from delivery_order do WHERE (do.bundle_id = dba.bundle_id OR do.console_number = dba.bundle_id) AND do.isactive= 'Y' AND do.kode_status = 'WT0'
      // ) > 0`;

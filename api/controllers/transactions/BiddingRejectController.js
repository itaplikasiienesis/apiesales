...
module.exports = {
    ...
    find: async function (req, res) {
        ...
        try {
          ...
          FROM listing_bidding_reject_v WHERE 1=1
         ...
         (SELECT TOP 1 CONVERT(VARCHAR(16),tanggal_gi,120)
          ...
          FROM listing_bidding_reject_v ltbv WHERE 1=1 ${WhereBundleId} ${whereRange} ${WherePlannerId} ${WhereTranspoterId} ${WhereKodeStatus} ${filtersearchtext}
          ...
          if(count == 0){
            ...
            queryDataTable = `SELECT * FROM listing_bidding_reject_v WHERE 1=1
            ${WhereBundleId} ${whereRange} ${WherePlannerId} ${WhereTranspoterId} ${WhereKodeStatus} ${filtersearchtext}
            ORDER BY tanggal_penjemputan ASC`;
      ...
      konfirmLogistik: async function (req, res) {
        ...
        try {
            ...
            let sqlGetChekbundleOrConsole = `SELECT COUNT(1) jumlah_data FROM delivery_order_v dov WHERE bundle_id = '${bundle_id}'`;
            let dataCheckBundleOrConsole = await request.query(sqlGetChekbundleOrConsole);
            let jumlah_data = dataCheckBundleOrConsole.recordset.length > 0 ? dataCheckBundleOrConsole.recordset[0].jumlah_data : 0;
                    ...
                    if (jml > 0) {
                      ...
                      if (getConsolNumber == null || !getConsolNumber ) {
                        ...
                        if(delivery_bundle_approval_id){
                          ...
                          const sql2 = `UPDATE delivery_bundle_approval SET flag = 1,isstatus = 2
                        ...
                        if(delivery_bundle_approval_id){
                          ...
                          const sql3 = `UPDATE delivery_bundle_approval SET flag = 1,isstatus = 2
                          WHERE delivery_bundle_approval_id = '${delivery_bundle_approval_id}'  `
                      ...
                      if (getConsolNumber == null || !getConsolNumber ) {
                        ...
                        let delivery_bundle_approval_id = dataApproval.recordset.length > 0 ? dataApproval.recordset[0].delivery_bundle_approval_id : null;
                         ...
                         if(delivery_bundle_approval_id){
                          ...
                          const updateDataApproval = `UPDATE delivery_bundle_approval SET flag = 2,isstatus = 2
                          WHERE delivery_bundle_approval_id = '${delivery_bundle_approval_id}'  `
...
updateDataTransporter: async function (req, res) {
  ...
  try {
    ...
    let sqlGetDataDoSpesifik = `SELECT a.r_kendaraan_id,rk.kode AS jenisKendaraan 
    FROM listing_bidding_reject_v a
    ...
    if(totalData == 0){
      ...
      request.query(queryUpdate, async (err, result) => {
        ...
        let sqlgetdo = `SELECT * FROM listing_bidding_reject_v WHERE nomor_id = '${nomor_id}'`;
  ...
  findJumlahBiddingReject: async function(req, res) {
    await DB.poolConnect;
    try {
      const request = DB.pool.request();

      let queryDataTable = `SELECT COUNT(1) AS jumlah FROM listing_bidding_reject_v`;

...
module.exports = async function(proposal_id) {
    ...
    try {
        ...
        let status_proposal = status_id==30 ? 'APPROVED': 'REJECTED';
          ...
          let paramEmailCreatedProposal = {
            
            subject:`E-PROPOSAL (Admin) ${proposal_no}  - YOUR PROPOSAL IS ${status_proposal}`,
        ...
        let sqlGetEmailCreatedBy = `SELECT email FROM employee e WHERE e.name='${createdproposal}' and email IS NOT NULL AND active=1`;
        console.log(sqlGetEmailCreatedBy);
        let dataemailcreatedby = await request.query(sqlGetEmailCreatedBy);
        let emailcreatedby = dataemailcreatedby[0];
        let dataemail = [];
        for (let i = 0; i < emailcreatedby.length; i++) {
              
            dataemail.push(emailcreatedby[i].email);
        ...
        if(dataemail.length > 0){
            ...
            let templateEmailCreateProposal = await sails.helpers.generateHtmlEmail.with({ htmltemplate: 'emailcreateproposal', templateparam: paramEmailCreatedProposal}); 
            SendEmail(dataemail.toString(), paramEmailCreatedProposal.subject, templateEmailCreateProposal,null,'EPROP');

...
module.exports = async function(proposal_id) {
    ...
    try {
        ...
        //let logo = 'data:image/jpeg;base64,' + Buffer.from(base64_encode(path.resolve(sails.config.appPath, 'assets', 'report', 'fkr', 'assets', 'logo.jpg')));
        ...
        for (let i = 0; i < eksekutorlist.length; i++) {
            ...
            let dataemail = [];
            let nama_eksekutor = eksekutorlist[i].name;
            let email = eksekutorlist[i].email;
            ...
            let paramEmailCreatedProposal = {
                ...
                subject:`E-PROPOSAL (Executor) ${proposal_no}  - YOUR PROPOSAL IS ${status_proposal}`,
            ...
            let templateCreatedProposal = await sails.helpers.generateHtmlEmail.with({ htmltemplate: 'emaileksekutor', templateparam: paramEmailCreatedProposal}); 
            SendEmail(dataemail.toString(), paramEmailCreatedProposal.subject, templateCreatedProposal,null,'EPROP');

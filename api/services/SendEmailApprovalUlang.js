...
module.exports = async function(proposal_id,no_appr,email) {
    ...
    try {
        ...
        let sqlGetApproval = `SELECT name,position_appr,status_approval_desc
        FROM v_appr_history WHERE proposal_id = '${proposal_id}' ORDER BY no_appr ASC`;
        ...
        for (let i = 0; i < budgetlist.length; i++) {
          ...
          budgetlist[i].budget = numeral(budgetlist[i].budget).format('0,0');
        ...
        let sqlgetAppovalPresent = `SELECT token,proposal_approval_id,a.employee_id,b.name,b.email,b.company_id
        FROM proposal_approval a
        LEFT JOIN employee b ON(a.employee_id=b.employee_id)
        WHERE a.proposal_id = '${proposal_id}'
        AND a.no_appr=${no_appr}
        and a.status_approval_id = 1`;
        let dataapprovalpresent = await request.query(sqlgetAppovalPresent);
        let approvalpresentlist = dataapprovalpresent[0];
        let dataemail = [];

        for (let i = 0; i < approvalpresentlist.length; i++) {

                        
            let proposal_appr = approvalpresentlist[i].proposal_approval_id;
            let employee_id = approvalpresentlist[i].employee_id;
            let nama_approval = approvalpresentlist[i].name;
            //let email = approvalpresentlist[i].email;
            let company_id = approvalpresentlist[i].company_id;

            console.log('email ',email);
            dataemail.push(email);
            let linkreject =`https://esales.enesis.com/api/proposalapprove_approve/${proposal_appr}/RJ/${employee_id}/${company_id}`;
            let linkapprove=`https://esales.enesis.com/api/proposalapprove_approve/${proposal_appr}/AP/${employee_id}/${company_id}`;

            let paramEmailApproval = {
                
                subject:`E-PROPOSAL ${proposal_no} - PROPOSAL NEED YOUR APPROVAL`,
            ...
            let dataemaildev = [];
            dataemaildev.push(email);
            dataemail.push('tiasadeputra@gmail.com');
            let templateApprovalProposal = await sails.helpers.generateHtmlEmail.with({ htmltemplate: 'emailapproval', templateparam: paramEmailApproval});
            SendEmail(dataemail.toString(), paramEmailApproval.subject, templateApprovalProposal,null,'EPROP');
